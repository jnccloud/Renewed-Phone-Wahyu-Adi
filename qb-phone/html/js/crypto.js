    ...
    $.post('https://qb-phone/GetCryptosFromDegens', JSON.stringify({}), function(Jobs){
        ...
        for (const [k, v] of Object.entries(Jobs)) {
            ...
            if (v.sell && v.purchase){
                var AddOption = 
                '<div class="crypto-list" id="crypto-id"><span class="crypto-icon"><i class="'+v.icon+'"></i></span> <span class="crypto-label">'+v.label+'</span> <span class="crypto-value">'+CryptoType[Crypto]+'</span>' +
...
$(document).on('click', '#crypto-send-purchase', function(e){
    ...
    var amount = $(".crypto-amount-buy").val();
    if(amount != ""){
        setTimeout(function(){
            ConfirmationFrame()
        }, 150);
